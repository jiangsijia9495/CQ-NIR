...
class SDFNetwork(nn.Module):
    ...
    def pts_loc(self, pts):
        ...
        embed = neighbor_indices[:, :, 0]*res*res + neighbor_indices[:, :, 1]*res + neighbor_indices[:, :, 2]
        ...
        weight = dxyz[:, :, :, 0] * dxyz[:, :, :, 1] * dxyz[:, :, :, 2]          # bs, npoints, 8
    ...
    def forward(self, inputs):
        ...
        _, center= self.pts_loc(inputs[None])
        inputs = center.squeeze()
    ...
    def forward_inside(self, inputs):
        inputs = inputs * self.scale
        if self.embed_fn_fine is not None:
            inputs = self.embed_fn_fine(inputs)
        ...
        for l in range(0, self.num_layers - 1):
            ...
            if l < self.num_layers - 2:
                x = self.activation(x)
        return torch.cat([x[:, :1] / self.scale, x[:, 1:]], dim=-1)[:, :1]
    def sdf(self, x):
    ...
    def gradient(self, x):
        _, center= self.pts_loc(x[None])
        x = center.squeeze()
        x.requires_grad_(True)
        y = self.forward_inside(x)
...
class RenderingNetwork(nn.Module):
    ...
    def forward(self, points, normals, view_dirs, feature_vectors):
        _, center= self.pts_loc(points[None])
        points = center.squeeze()
